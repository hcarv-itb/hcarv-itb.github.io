...
# 	
#     theoretic_df=pd.DataFrame({f'initial={c}':P_t, f'[bulk]= {np.round(c_opt[0], decimals=3)}$\pm${np.round(stdev, decimals=3)}{unit}':P_opt}, index=dG.index.values)
#     dG=pd.concat([dG, dG_err.abs()], axis=1)
#     #dG.rename(columns={c:f'[mol] = {np.round(c_opt[0], decimals=1)} {unit}'., 0:'err-{}'.format(c)}, inplace=True)
